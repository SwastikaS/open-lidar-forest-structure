# Batch validation of hybrid TLS DBH estimation
...
batch_inventory <- data.frame(
  tree_id = toupper(sub("_.*$", "", basename(tls_files))),
  file_path = tls_files,
  stringsAsFactors = FALSE
)

cat("Trees prepared for batch processing:", nrow(batch_inventory), "\n")
...
batch_inventory$match_id <-
  standardise_tree_id(batch_inventory$tree_id)
...
batch_inventory$species <-
  reference$species[reference_row]

batch_inventory$reference_dbh_cm <-
...
batch_results <- vector(
  "list",
  nrow(batch_inventory)
)

for (i in seq_len(nrow(batch_inventory))) {
  ...
  cat(
    ...
    nrow(batch_inventory),
    ":",
    batch_inventory$tree_id[i],
    "\n"
  )

  batch_results[[i]] <- tryCatch(
    {
      fit <- estimate_tree_dbh(
        batch_inventory$file_path[i]
      )

      data.frame(
        tree_id = batch_inventory$tree_id[i],
        species = batch_inventory$species[i],
        method_used = fit$method_used,
        reference_dbh_cm =
          batch_inventory$reference_dbh_cm[i],
        estimated_dbh_cm =
...
batch_results$quality_flag <- ifelse(
  batch_results$processing_status == "failed",
  "failed",
  ifelse(
    batch_results$circle_rmse_mm <= 20 &
      batch_results$circumference_completeness_percent >= 70 &
      batch_results$retained_points >= 100,
    "acceptable",
    "inspect"
  )
)
...
batch_summary <- data.frame(
  ...
  fits_requiring_inspection = sum(
    batch_results$quality_flag == "inspect",
    na.rm = TRUE
  ),
  mean_absolute_error_cm = mean(
    batch_results$absolute_error_cm[
      successful_rows
    ],
    na.rm = TRUE
  ),
...
write.csv(
  batch_summary,
  "outputs/tls/tables/tls_dbh_batch_summary.csv",
  row.names = FALSE
)
